...
public class DefaultProvisioningService implements ProvisionService,  ApplicationContextAware {
	...
	public ProvisionUserResponse addUser(ProvisionUser user) {
		...
		IdmAuditLog auditLog = auditHelper.addLog("CREATE", user.getSecurityDomain(),	primaryLogin.getId().getLogin(),
				"IDM SERVICE", user.getCreatedBy(), "0", "USER", user.getUserId(), 
				null,   "SUCCESS", null,  "USER_STATUS", 
				user.getUser().getStatus().toString(),
				requestId, null, user.getSessionId());
		...
		if (resourceList != null) {
			for ( Resource res : resourceList) {
				...
				auditHelper.addLog("ADD IDENTITY", mLg.getId().getDomainId(),	mLg.getId().getLogin(),
						...
						"USER", user.getUserId(), 
						auditLog.getLogId(),   "SUCCESS", null,  "USER_STATUS", 
						user.getUser().getStatus().toString(),
						requestId, null, user.getSessionId());
			...
			for (Login l : principalList) {
				if (l.getStatus() != null && !l.getStatus().equalsIgnoreCase("INACTIVE")) {
					...
					if (!l.getId().getManagedSysId().equalsIgnoreCase(this.sysConfiguration.getDefaultManagedSysId())) {
			            ...
			            auditHelper.addLog("DELETE IDENTITY", l.getId().getDomainId(),	l.getId().getLogin(),
	...
	public ProvisionUserResponse deleteUser(String securityDomain,
		...
		if (!managedSystemId.equalsIgnoreCase(sysConfiguration.getDefaultManagedSysId())) {
            ...
            auditHelper.addLog("DELETE", usr.getSecurityDomain(),	login.getId().getLogin(),
					"IDM SERVICE", usr.getCreatedBy(), login.getId().getManagedSysId(),
					"USER", usr.getUserId(), 
					null,   "SUCCESS", null,  "USER_STATUS", 
					usr.getStatus().toString(),
					requestId, null, null);
				...
				for (Login l : principalList) {
					if (l.getStatus() != null && !l.getStatus().equalsIgnoreCase("INACTIVE")) {
						...
						if (!l.getId().getManagedSysId().equalsIgnoreCase(this.sysConfiguration.getDefaultManagedSysId())) {
				            ...
				            auditHelper.addLog("DELETE", usr.getSecurityDomain(),	l.getId().getLogin(),
									"IDM SERVICE", usr.getCreatedBy(), l.getId().getManagedSysId(),
									"USER", usr.getUserId(), 
									auditLog.getLogId(),   "SUCCESS", null,  "USER_STATUS", 
									usr.getStatus().toString(),
									requestId, null,null);
	...
	public ProvisionUserResponse modifyUser(ProvisionUser pUser) {
		...
		if (resourceList != null) {
			...
			for ( Resource res : resourceList) {
	            ...
	            if (res.getObjectState().equalsIgnoreCase(BaseObject.NEW)) {
	            //if (mLg == null) {
		           ...
		           // mLg.setPassword(primaryLogin.getPassword());
					...
					bindingMap.remove(matchObj);
				 	...
				 	if (mLg != null) {
						...
						log.debug("LOGIN ID: " + mLg.getId().getLogin());
			            //
						PSOIdentifierType idType = new PSOIdentifierType(mLg.getId().getLogin(),null, "target");
			            ...
			            if (mLg.getOperation() == AttributeOperationEnum.REPLACE && mLg.getOrigPrincipalName() != null ) {
			            	extAttList.add(new ExtensibleAttribute("ORIG_IDENTITY", mLg.getOrigPrincipalName(), 2 ));
			            }
