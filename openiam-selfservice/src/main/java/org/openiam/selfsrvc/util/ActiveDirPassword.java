...
public class ActiveDirPassword implements Password {
	...
	public NamingEnumeration search(LdapContext ctx ,String filterAttr, String filterValue, String baseDN) {
	...
	public void setPassword(ConnectionParam param, String loginId,String password) {
		String name=null, sn=null, email=null, firstName=null, dn=null;
		
		try {
			LdapContext ctx = connection.connect(param);
			
			// we should be searching either the sAMAccountName or the UserPrincipalName. Both of these values are
			// unique
			NamingEnumeration answer = search(ctx,res.getString("searchAttribute"), loginId, param.getBaseDN());
