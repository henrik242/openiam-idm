...
public class UserMgrPhoneTest extends AbstractOpenIAMTestCase  {
	...
	public void testAppPhones() {
		User newUser = userMgr.getUserWithDependent("4917",true);
		
		assertNotNull(newUser);
		
		Set<Phone> phSet = newUser.getPhone();
		assertNotNull(phSet);
		assertEquals(phSet.size(),6);
		...
		while (phoneIterator.hasNext()) {
			...
			userMgr.updateUserWithDependent(newUser, true);

			User newUser2 = userMgr.getUserWithDependent("4917",true);
			
			assertNotNull(newUser);
			
			Set<Phone> phSet2 = newUser2.getPhone();
			assertNotNull(phSet2);
			assertEquals(phSet2.size(),6);
	
	}
	
	public void testAddUserWithPhone() {

		user2.getPhone().add(phone1);
