	...
	public Subject login(AuthenticationContext authContext) throws AuthenticationException {
		...
		// if found that get the full name, add the password and try to authenticate.
		 ...
		 if (distinguishedName == null) {
			 log("AUTHENTICATION", "AUTHENTICATION", "FAIL", "RESULT_INVALID_LOGIN", domainId, null, principal, null, null);
			throw new AuthenticationException(AuthenticationConstants.RESULT_INVALID_LOGIN);
		
		 }
		 String dn = "uid=" + distinguishedName + "," + this.baseDn;
		 log.info("dn=" + dn);
		 
		 // try to login to AD with this user
		 LdapContext tempCtx = connect(dn, password);
		 if (tempCtx == null) {
