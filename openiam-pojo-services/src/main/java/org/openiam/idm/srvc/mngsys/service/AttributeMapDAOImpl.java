...
public class AttributeMapDAOImpl implements AttributeMapDAO {
	 ...
	 */
	public AttributeMap add(AttributeMap instance) {
	...
	public List<AttributeMap> findAllAttributeMaps() {
		Session session = sessionFactory.getCurrentSession();
		Query qry = session.createQuery("from org.openiam.idm.srvc.mngsys.dto.AttributeMap a " +
				" order by a.resourceId asc");
		qry.setCacheable(true);
		qry.setCacheRegion("query.attributeMap.findAllAttributeMaps");
		List<AttributeMap> attributeMaps = (List<AttributeMap>)qry.list();
