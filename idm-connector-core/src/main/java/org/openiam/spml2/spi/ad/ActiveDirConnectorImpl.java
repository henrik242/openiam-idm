...
public class ActiveDirConnectorImpl extends AbstractSpml2Complete implements ConnectorService  {
	...
	*/
	...
	public AddResponseType add(AddRequestType reqType) {
		...
		/* Need to support reference types - to allow member of Group functionality */ 
...
/*		if (reqType != null) {
			System.out.println("Request Id" + reqType.getRequestID());
			response.setRequestID(reqType.getRequestID());
		}else{
			System.out.println("Request type is null");
		}
		response.setStatus(StatusCodeType.SUCCESS);
		
		ExtensibleType extType = reqType.getData();
		System.out.println("Ext Type=" + extType);
*/		
		return response;
	}
