...
public class UserSearchAction extends DispatchActionSupport {
	...
	public ActionForward view(ActionMapping mapping, ActionForm form,
		...
		if (mode != null && mode.length() > 0) {
			request.setAttribute("msg", "Information has been successfully submitted for processing. ");
		}
	...
	private UserSearch createSearch(DynaValidatorForm form) {
        ...
        if (form.get("principalName") != null
				&& ((String) form.get("principalName")).length() > 0) {
			search.setPrincipal(((String) form.get("principalName")) + "%");
		}
