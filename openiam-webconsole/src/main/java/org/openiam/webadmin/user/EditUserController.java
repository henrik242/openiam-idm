...
import org.openiam.base.ExtendCommand;
...
public class EditUserController extends SimpleFormController {
	...
	protected ModelAndView onSubmit(HttpServletRequest request,
		...
		log.info("User=" + usr);
		log.info("showinsearch=" + usr.getShowInSearch());
		...
		if (btnName.equalsIgnoreCase("LOCK")) {
			provRequestService.lockUser(usr.getUserId(), AccountLockEnum.LOCKED_ADMIN, userId);
			return new ModelAndView(new RedirectView(redirectView+"&mode=1", true));
		}
		if (btnName.equalsIgnoreCase("UN-LOCK")) {
			provRequestService.lockUser(usr.getUserId(), AccountLockEnum.UNLOCK, userId);
			return new ModelAndView(new RedirectView(redirectView+"&mode=1", true));			
		}
		...
		if (btnName.equalsIgnoreCase("DELETE")) {
			...
			ExtendCommand extCmd = (ExtendCommand)se.instantiateClass(null, "/ui/webconsole/EditUserController.groovy");
			extCmd.pre("DELETE", pUser);
			
			System.out.println("Script: =" + pUser.getCompanyId());
