...
public class ManagedSysConnectionController extends SimpleFormController {
	...
	/*	if (btn != null && btn.equalsIgnoreCase("Test Connection")) {
			...
			ProvisionConnector connector = connectorService.getConnector(sysConCommand.getConnectorId());
			...
			boolean result = port.testConnection(sysConCommand.getManagedSysId());
			
			ModelAndView mav = new ModelAndView("/testresult");
	        
			if (result) {
				mav.addObject("msg", "Connection was successful");
			}else {
				mav.addObject("msg", "Connection failed.");
			}
			mav.addObject("sysId", sysConCommand.getManagedSysId());
			request.setAttribute("menuGroup", null);
	        return mav;
